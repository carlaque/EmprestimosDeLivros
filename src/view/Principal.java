package view;

import java.io.IOException;

import controllers.Aluno;
import controllers.Emprestimo;
import controllers.Livro;
import controllers.Professor;
import model.Alunos;
import model.Devolucoes;
import model.Emprestimos;
import model.Livros;
import model.Professores;

public class Principal {
	public static void main(String[] args) throws IOException {
/*		Livros livros = new Livros();
		Alunos alunos = new Alunos();
		Professores professores = new Professores();
		Devolucoes devolucoes = new Devolucoes();
		Emprestimos emprestimos = new Emprestimos(livros, alunos, professores, devolucoes);
		
		String[] nomes = {"francisco", "bernado", "clarice", "guilherme", "marilia"};
		
		for(int i =0;i < 5; i ++) {
			Professor a = new Professor();
			a.cadastrar(professores.getProximoCodigo(), nomes[i], "rua xxx", "email@email.com", "123456789", "todos", "1234564879", 0);
			professores.insereCadastro(a);
		}		
		
		Emprestimo novo = new Emprestimo();
		if(novo.Emprestar(emprestimos.getProximoCodigo(), livros.buscaPeloCodigo(1), alunos.buscaPeloCodigo(4)))
			emprestimos.insereCadastro(novo);
		
		try {
			CadastroAluno frame = new CadastroAluno(alunos);
			frame.setVisible(true);
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		//TODO: VERFICACAO SE EXISTE EMPRESTIMO ANTES DE DEVOLVER
		Emprestimo novoEmprestimo = emprestimos.buscaPeloCodigo(4);
		novoEmprestimo.devolverLivro(devolucoes.getProximoCodigo());
		devolucoes.insereCadastro(novoEmprestimo.getDevolucao());

		
		emprestimos.salvarLista();
		livros.salvarLista();
		professores.salvarLista();
		alunos.salvarLista();
		devolucoes.salvarLista();
	*/
	}

}
